...
int main(){
	...
	int msgid_params;
	param_key = param_send_chasier1;
	msgid_params = msgget(param_key, 0666 | IPC_CREAT);
	//load parameter
	while(1){
		clear();
		printf("CHASIER 1 NODE, PARAM LOAD, WAITING..\n");
		message.mesg_type = 1;
		...
		sleep(3);
		printf("CHASIER 1 NODE, PARAM LOADED ! \n");
		sleep(1);
		break;
	...
	while(1){
		...
		printf("[INFO] Runing... \n");
	        clear();  
	        msgctl(msgid_chasier1, IPC_STAT, &buf);
	        if (fristStages>=1){
	        for (int timer=0; timer<=delay; timer++){
				...
				red();
				strcpy(infoChasier,"Ups, CHASIER 1 IS FULL!"); 
			
			}else{
			   isOverflow = 0;
			  reset();
			  strcpy(infoChasier,"CHASIER 1 GOOD!"); 
			...
			printf("[INFO] THE CHASIER 1 SITUATION INFO (GOOD/OVERFLOW)   ->> %s\n", infoChasier);
			...
			*/
	        ...
	        flag = msgrcv(msgid_chasier1, &message1, sizeof(message), 0, 0);
