...
int main(){
	...
	int msgid_params;
	param_key = param_send_eating;
	msgid_params = msgget(param_key, 0666 | IPC_CREAT);
	//load parameter
	while(1){
		clear();
		printf("EATING ROOM  NODE, PARAM LOAD, WAITING..\n");
		message.mesg_type = 1;
		...
		sleep(3);
		printf("EATING ROOM NODE, PARAM LOADED ! \n");
		sleep(1);
		break;
