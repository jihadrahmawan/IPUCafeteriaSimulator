...
int main(){
	...
	int msgid_params;
	param_key = param_send_foodpick;
	msgid_params = msgget(param_key, 0666 | IPC_CREAT);
	//load parameter
	while(1){
		clear();
		printf("FOOD PICK NODE, PARAM LOAD, WAITING..\n");
		message.mesg_type = 1;
		...
		sleep(3);
		printf("FOOD PICK NODE, PARAM LOADED ! \n");
		sleep(1);
		break;
